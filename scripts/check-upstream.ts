...
const current = String(pkg.dependencies['fluentui-web-icons'] ?? '').replace(/^[^\d]*/, '');
const latest = await npmLatest('fluentui-web-icons');
...
console.log(`fluentui-web-icons ${current} → ${latest}; package ${pkg.version}; npm ${listed ? (alreadyPublished ? 'has' : 'missing') : 'has never listed'} ${latest}; release=${release}`);
...
else if (!release) console.log('Already in sync with fluentui-web-icons.');

async function npmLatest(name: string): Promise<string> {
  const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(name)}/latest`, { headers: { accept: 'application/json', 'user-agent': 'fluentui-astro-icons-sync' } });
...
async function npmListed(name: string): Promise<boolean> {
  const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(name)}`, { headers: { accept: 'application/json', 'user-agent': 'fluentui-astro-icons-sync' } });
