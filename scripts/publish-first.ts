...
process.env.GITHUB_REPOSITORY ||= 'l5z12/fluentui-astro-icons';
...
async function npmListed(packageName: string): Promise<boolean> {
  const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(packageName)}`, { headers: { accept: 'application/json', 'user-agent': 'fluentui-astro-icons-sync' } });
...
async function npmHasVersion(packageName: string, packageVersion: string): Promise<boolean> {
  const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(packageName)}/${encodeURIComponent(packageVersion)}`, { headers: { accept: 'application/json', 'user-agent': 'fluentui-astro-icons-sync' } });
  if (response.status === 404) return false;
